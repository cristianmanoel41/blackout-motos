 ...
 *
 * Um endereço público em CSV que o Commerce Manager busca
 * sozinho, de hora em hora. Com ele o anúncio deixa de ser
 * uma imagem solta: aparece a moto certa, com preço, e a
 * conversa no WhatsApp já começa falando dela.
 *
...
function descricao(moto: MotoSite) {
  ...
  if (selos.length > 0) {
    partes.push(
      `${selos.join(", ").replace(/^./, (l) => l.toUpperCase())}.`
    );
  }
 ...
 * image_link e brand; o resto ajuda a segmentar campanha
 * sem precisar mexer no anúncio depois.
 */
const COLUNAS = [
  "id",
  "title",
